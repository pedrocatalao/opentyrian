...
// Keyboard scancodes, modifier flags and mouse buttons as the game sees them.
// The numbering is SDL2's (USB HID usage page 7, with SDL's additions), so
// the SDL platform layer passes them through unchanged and saved key
// configurations stay compatible.  Other platform layers map into this space.
// Derived from SDL_scancode.h (zlib license, Copyright (C) Sam Lantinga).
...
/*     SCANCODE_LOCKINGSCROLLLOCK = 132, */
    ...
    SCANCODE_COUNT = 512 /**< not a key, just marks the number of scancodes
                                 for array bounds */
} Scancode;

// Modifier flags, same values as SDL_Keymod.
enum
{
	MOD_NONE   = 0x0000,
	MOD_LSHIFT = 0x0001,
...
// Mouse buttons, same values as SDL's.
enum
{
	MOUSE_BUTTON_LEFT = 1,
...
#define MOUSE_BUTTON_MASK(button) (1 << ((button) - 1))
...
// Human-readable names, identical to SDL_GetScancodeName(), used in the key
// configuration screen and in saved configuration files.
#include <stdbool.h>

const char *scancode_name(Scancode scancode);
Scancode scancode_from_name(const char *name);

// The character a key produces on a US layout, or 0.
