...
static SDL_PixelFormat *main_window_tex_format = NULL;
static SDL_Texture *main_window_texture = NULL;

// The current palette in the texture's pixel format and in YUV, for the
// scalers.  Rebuilt on every present from the game's palette.
...
static void calc_dst_render_rect(const Surface *src_surface, SDL_Rect *dst_rect);
static void scale_and_flip(const Surface *);
...
static void calc_dst_render_rect(const Surface *const src_surface, SDL_Rect *const dst_rect)
{
...
void plat_present(const Surface *screen, const Color palette[256])
{
	...
	scale_and_flip(screen);
}

static void scale_and_flip(const Surface *src_surface)
{
	// Do software scaling
	assert(scaler_function != NULL);
	scaler_function((Surface *)src_surface, main_window_texture);
